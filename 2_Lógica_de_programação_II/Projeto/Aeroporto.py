...
pista1_geral = {'nome': "Pista 1", 'Fila de decolagem': [], 'Fila de aterrisagem': [], 'Pousos de emergência': 0, 'estado': 'livre'}
pista2_geral = {'nome': "Pista 2", 'Fila de decolagem': [], 'Fila de aterrisagem': [], 'Pousos de emergência': 0, 'estado': 'livre'}
pista3_geral = {'nome': "Pista 3", 'Fila de decolagem': [], 'Fila de aterrisagem': [], 'Pousos de emergência': 0, 'estado': 'livre'}
...
def tamanho_maximo_avioes(pista_a: dict, pista_b: dict, pista_c: dict ): 
    ...
    esses valores.
    '''
    tamanho_decolagem = tamanho(fila = 'Fila de decolagem', pista_aa = pista_a, pista_bb = pista_b, pista_cc = pista_c)
    tamanho_aterrisagem = tamanho('Fila de aterrisagem', pista_aa = pista_a, pista_bb = pista_b, pista_cc = pista_c)
...
def define_id(numero_id_local: int):
    ...
    id += str(numero_id_local)
    numero_id_local += 1
    
    while contador < 3:
        id += letras[random.randint(0,25)]
        contador += 1
    return numero_id_local, id

def gerar_aviao(tamanho_maximo: int, avioes_gerados_local: list, numero_id_local2: int):
    ...
    '''    
    quantidade_randomica = random.randint(0,tamanho_maximo)
    contador = 1 # Alterei a variável para gerar a qauntidade de aviões certa! 

    while contador <= quantidade_randomica:
        numero_id_local2, id = define_id(numero_id_local = numero_id_local2)
        avioes_gerados_local.append({'ID' : id, 'combustível' : random.randint(2,20), 
        'tempo espera decolagem': 0, 'tempo espera aterrissagem': 0, 'tempo limite': 1})
        contador += 1
    return avioes_gerados_local, numero_id_local2

def adicionar_aviao_aterrisagem(pista: dict, avioes_gerados_local2: list):
    ...
    while contador > 0:
        if len(avioes_gerados_local2) > 0: 
            aviao = avioes_gerados_local2.pop(0)
            pista['Fila de aterrisagem'].append(aviao)
        else: 
            break
        contador -=1
    return pista, avioes_gerados_local2

def mover_para_fila(operacao: str, avioes_gerados_local: list, pista_a: dict, pista_b: dict, pista_c: dict):
    ...
    if operacao == 'Decolagem':
        for aviao in avioes_gerados_local: 
            pista_b['Fila de decolagem'].append(aviao)
        avioes_gerados_local.clear() # apagando itens da lista
    elif operacao == 'Aterrisagem': 
        pista_c, avioes_gerados_local = adicionar_aviao_aterrisagem(pista = pista_c, avioes_gerados_local2 = avioes_gerados_local)
        pista_a, avioes_gerados_local = adicionar_aviao_aterrisagem(pista = pista_a, avioes_gerados_local2 = avioes_gerados_local)
        pista_b, avioes_gerados_local = adicionar_aviao_aterrisagem(pista = pista_b, avioes_gerados_local2 = avioes_gerados_local)
        avioes_gerados_local.clear()
    return pista_a, pista_b, pista_c, avioes_gerados_local
...
def alterar_estado(pista_local: dict, novo_estado: str) -> dict:
    ...
    aterrissando na pista2.
    '''
    pista_local['estado'] = novo_estado
    return pista_local

def decolar(pista: dict, tempos_decolagem_local: list):
    ''' Tem como parâmetro a pista2, que é utilizada para decolar os aviões, removendo 
    ...
    '''
    if len(pista['Fila de aterrisagem']) == 0 and len(pista['Fila de decolagem']) > 0: #Acrescentei a condição de and para não dar erro de lista vazia
...
def tempo_espera(pista: dict, fila:str, tempo: str) -> dict:
    ''' Tem três parâmetros, sendo a pista em que o avião está, a fila 
    ...
    if len(pista_a['Fila de aterrisagem']) > 0:
        if pista_a['Fila de aterrisagem'][0]['tempo limite'] > 1:
            if len(pista_a['Fila de aterrisagem']) <= 2: 
                ...
                if len(pista_b['Fila de aterrisagem']) == 3: # Faz a troca, ao invés de somente inserir
                    if pista_b['Fila de aterrisagem'][1]['tempo limite'] == 1:
                        aviao = pista_b['Fila de aterrisagem'].pop(1)
                        pista_a['Fila de aterrisagem'].insert(0,(aviao))
                        aviao2 = pista_a['Fila de aterrisagem'].pop(3)
                        pista_b['Fila de aterrisagem'].append(aviao2)
                if len(pista_b['Fila de aterrisagem']) == 2: # Faz a troca, ao invés de somente inserir
                    if pista_b['Fila de aterrisagem'][1]['tempo limite'] == 1:
                        aviao = pista_b['Fila de aterrisagem'].pop(1)
                        pista_a['Fila de aterrisagem'].insert(0,(aviao))
                        aviao2 = pista_a['Fila de aterrisagem'].pop(3)
...
def aterrisar(pista: dict, tempos_aterrisagem_local: list):
    ''' Tem como parâmetro a pista, que será utilizado para acessar a fila de aterrissagem e 
    caso atendas as condições verificadas, o avião será removido de fila de aterrissagem. 
    Ela também é responsável por verificar se o avião está pousando em emergência e contar
    quantos aviões pousaram em emergência nessa pista.
    '''
...
def reduz_combustivel(pista: dict) -> dict: 
    ''' Responsável por percorrer a fila de aterrissagem da pista informada e 
    decrementar a chave combustível de todos os aviões. 
    '''
...
def exibir_status(pista: dict, operacao: str):
    ...
    Percorre a pista informada e acessa algumas chaves para retornar o painel de informaçõas 
    ...
    '''
    print(f"{pista['nome']}: \n{' '*2}Quantidade de aviões na fila {operacao}: {len(pista['Fila de ' + operacao])}\n"\
        f"{' '*2} Estado da pista: {pista['estado']}\n"
...
def calcular_medias(lista: list, operacao: str):
    ''' Tem como parâmetro lista, que pode ser dos tempos de decolagem ou aterrissagem. 
    Imprime o tempo médio de decolagem ou aterrissagem, que é definido pelo segundo 
    parâmetro da função. 
    '''
    if len(lista) > 0:
        print(f'Tempo médio de {operacao}: {(sum(lista)/len(lista)):.2f} {" rodada" if sum(lista)/len(lista) == 1 else " rodadas"}\n')
        return None

def cria_arquivo(total_queda_avioes: float, pousos_emergenciais: float, espera_media_decolagem: float, espera_media_aterrissagem: float, total_rodadas: float): 
    with open(file = 'dados aeroporto.csv', mode = 'a', encoding = 'UTF-8') as linha: 
        linha_1 = f"Total de queda de avioes; QTDE de pousos emergenciais; Tempo medio de espera para decolagem; Tempo medio de espera para aterrissagem; Total de rodadas\n"
        linha_2 = f'{total_queda_avioes};{pousos_emergenciais};{espera_media_decolagem:.2f};{espera_media_aterrissagem:.2f};{total_rodadas}\n'
        linha.write(linha_1)
        linha.write(linha_2)

tempo = int(input("Digite a quantidade de rodadas desejada:\n"))

def inicia_tempo(tempo: int, numero_id: int, pista1: dict, pista2: dict, pista3: dict,\
    ...
    '''
    rodada = 1
    total_queda_avioes = 0
    while tempo > 0:
        print(f"{'='*70}") 
        print(f" Estamos na rodada {rodada}")
        tamanho_maximo_decolagem, tamanho_maximo_aterrisagem = tamanho_maximo_avioes(pista_a = pista1, pista_b = pista2, pista_c = pista3)
        #print(f"aterrisagem = {tamanho_maximo_aterrisagem}, decolagem = {tamanho_maximo_decolagem}")
        avioes_gerados, numero_id = gerar_aviao(tamanho_maximo = tamanho_maximo_decolagem, avioes_gerados_local = avioes_gerados, numero_id_local2 = numero_id)
        pista1, pista2, pista3, avioes_gerados = mover_para_fila(operacao = 'Decolagem', avioes_gerados_local = avioes_gerados, pista_a = pista1, pista_b = pista2, pista_c = pista3)
        avioes_gerados, numero_id = gerar_aviao(tamanho_maximo = tamanho_maximo_aterrisagem, avioes_gerados_local = avioes_gerados, numero_id_local2 = numero_id)
        pista1, pista2, pista3, avioes_gerados = mover_para_fila(operacao = 'Aterrisagem', avioes_gerados_local = avioes_gerados, pista_a = pista1, pista_b = pista2, pista_c = pista3)

        pista2, tempos_decolagem = decolar(pista = pista2, tempos_decolagem_local = tempos_decolagem)
        
        pista1 = tempo_espera(pista = pista1, fila = 'Fila de decolagem', tempo = 'tempo espera decolagem')
        pista2 = tempo_espera(pista = pista2, fila = 'Fila de decolagem', tempo = 'tempo espera decolagem')
        pista3 = tempo_espera(pista = pista3, fila = 'Fila de decolagem', tempo = 'tempo espera decolagem')
        ...
        pista1, tempos_aterrisagem = aterrisar(pista = pista1, tempos_aterrisagem_local = tempos_aterrisagem)
        pista2, tempos_aterrisagem = aterrisar(pista = pista2, tempos_aterrisagem_local = tempos_aterrisagem)
        pista3, tempos_aterrisagem = aterrisar(pista = pista3, tempos_aterrisagem_local = tempos_aterrisagem)
        
        pista1 = tempo_espera(pista = pista1, fila = 'Fila de aterrisagem', tempo = 'tempo espera aterrissagem')
        pista2 = tempo_espera(pista = pista2, fila = 'Fila de aterrisagem', tempo = 'tempo espera aterrissagem')
        pista3 = tempo_espera(pista = pista3, fila = 'Fila de aterrisagem', tempo = 'tempo espera aterrissagem')
        ...
        total_queda_avioes = total_queda_avioes + total_quedas1 + total_quedas2 + total_quedas3 
        ...
        tempo -= 1
        rodada +=1
        #pause = input("Presseione Enter para ir para próxima rodada.")

    if tempo == 0: 
        print('Fim do programa!')
    pousos_emergenciais = pista1["Pousos de emergência"] + pista2["Pousos de emergência"] + pista3["Pousos de emergência"]
    tempo_espera_decolagem = sum(tempos_decolagem)/len(tempos_decolagem)
    tempo_espera_aterrissagem = sum(tempos_aterrisagem)/len(tempos_aterrisagem)
    
    return total_queda_avioes, pousos_emergenciais, tempo_espera_decolagem, tempo_espera_aterrissagem, (rodada-1) 

total_queda_avioes_geral, pousos_emergenciais_geral, espera_media_decolagem_geral, espera_media_aterrissagem_geral, total_rodadas_geral = inicia_tempo(tempo = tempo, numero_id = numero_id_geral, pista1 = pista1_geral, pista2 = pista2_geral, pista3 = pista3_geral, tempos_decolagem = tempos_decolagem_geral, tempos_aterrisagem = tempos_aterrisagem_geral, avioes_gerados = avioes_gerados_geral)
cria_arquivo(total_queda_avioes = total_queda_avioes_geral, pousos_emergenciais = pousos_emergenciais_geral, espera_media_decolagem = espera_media_decolagem_geral, espera_media_aterrissagem = espera_media_aterrissagem_geral, total_rodadas = total_rodadas_geral)
